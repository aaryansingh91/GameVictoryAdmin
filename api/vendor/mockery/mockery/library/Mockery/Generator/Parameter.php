...
class Parameter
{
    ...
    {
        return new DefinedTargetClass($this->rfp->getType()->getName());
    }
    ...
    public function getTypeHintAsString()
    {
        ...
        if ($this->rfp->getType() && $this->rfp->getType()->getName() === 'array') {
            return 'array';
        }
        ...
        if ((version_compare(PHP_VERSION, '5.4.1') >= 0)) {
            try {
                if ($this->rfp->getType()->getName()) {
                    return $this->getOptionalSign() . $this->rfp->getType()->getName();
                }
